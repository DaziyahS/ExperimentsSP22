...
public:
    ...
    std::vector<std::vector<int>> list = {{0, 0}, {0, 1}, {0, 2}, {0, 3}, 
                                          {1, 0}, {1, 1}, {1, 2}, {1, 3},
                                          {2, 0}, {2, 1}, {2, 2}, {2, 3}};
    ...
    virtual void update() override
    {
        ...
        else if (screen_name == "trans_screen_1")
        {
            transScreen1();
            first_in_trial = true;
        }
        else if (screen_name == "trial_screen_1")
        {
           // trialScreen1();
        }
        else if (screen_name == "trans_screen_2")
        {
            transScreen2();
            first_in_trial = true;
        }
        else if (screen_name == "trial_screen_2")
        {
            trialScreen2();
        }
...
void beginScreen()
{
    ...
    if(ImGui::BeginPopup("subject_num")) // if clicked essentially
    {
        ...
        ImGui::InputText("##edit", name, IM_ARRAYSIZE(name), ImGuiInputTextFlags_CharsNoBlank); // size wanted and only numbers           
...
void transScreen1()
{
    ...
    ImGui::Text("During the following experiment, you will receive two haptic cues and be asked");
    ImGui::Text("to distinguish between the two. There will be a learning portion and then you");
    ImGui::Text("will be asked to move on and determine which signal you are receiving.");
    ImGui::Text(" ");
